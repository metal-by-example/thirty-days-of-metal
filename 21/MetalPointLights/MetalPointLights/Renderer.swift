...
class Renderer: NSObject, MTKViewDelegate {
    ...
    func makeScene() {
        ...
        lights = [ambientLight] + orbitLights
    }
    ...
    func updateLightConstants() {
        ...
        for i in 0..<orbitLights.count {
            let lightAngleOffset = ((2 * Float.pi) / Float(orbitLights.count)) * Float(i)
            let lightAngle = t + lightAngleOffset

            let orbitLightTransform = simd_float4x4(translate: SIMD3<Float>(orbitRadius * cos(lightAngle),
                                                                            1.0,
    ...
    func updateNodeConstants() {
        ...
        let rotation = simd_float4x4(rotateAbout: rotationAxis, byAngle: 270.0 * (180.0 / .pi))
