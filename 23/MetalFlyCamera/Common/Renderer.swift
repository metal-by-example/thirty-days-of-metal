...
class Renderer: NSObject, MTKViewDelegate {
    ...
    func makeScene() {
        ...
        for asteroidID in 1...3 {
            ...
            let asteroidsPerMesh = 333
            var batch = [Node]()
            for _ in 0..<asteroidsPerMesh {
                let asteroid = AsteroidState()
                asteroid.position = SIMD3<Float>(Float.random(in: -30...30),
                ...
                asteroid.angularVelocity = Float.random(in: -1.5...1.5)
                asteroid.scale = Float.random(in: 0.8...2.2)
    ...
    func updateNodeConstants(_ timestep: Float) {
        ...
        for (asteroidIndex, asteroidState) in asteroids.enumerated() {
            ...
            let s = asteroidState.scale
            let R = simd_float4x4(rotateAbout: asteroidState.rotationAxis,
                                  byAngle: asteroidState.rotationAngle)
            let t = asteroidState.position
            let transform = simd_float4x4(s * R.columns.0,
                                          s * R.columns.1,
                                          s * R.columns.2,
                                          SIMD4<Float>(t, 1))

            asteroidNodes[asteroidIndex].transform = transform
        }
