...
extension float4x4 {
    ...
    init(translation: SIMD3<Float>, orientation: simd_quatf, scale: SIMD3<Float>) {
        let R = float3x3(orientation)
        self.init(
            SIMD4<Float>(scale.x * R.columns.0, 0),
            SIMD4<Float>(scale.y * R.columns.1, 0),
            SIMD4<Float>(scale.z * R.columns.2, 0),
