...
class Renderer: NSObject, MTKViewDelegate {
    ...
    var tessellationFactor: Float = 2.0
    ...
    private var tessellationFactorOffset = 0
    ...
    func makePipelines() {
        ...
        renderPipelineDescriptor.vertexFunction = library.makeFunction(name: "vertex_displace_quad")!
    ...
    func updateNodeConstants(_ timestep: Float) {
        ...
        let factorsLayout = MemoryLayout<MTLQuadTessellationFactorsHalf>.self
        tessellationFactorOffset = allocateConstantStorage(size: factorsLayout.size, alignment: factorsLayout.stride)
        let factors = constantBuffer.contents().advanced(by: tessellationFactorOffset)
            .assumingMemoryBound(to: MTLQuadTessellationFactorsHalf.self)

        let factor = UInt16(representingAsFloat16: tessellationFactor)
        factors.pointee.edgeTessellationFactor = (factor, factor, factor, factor)
        factors.pointee.insideTessellationFactor = (factor, factor)
    }
    ...
    func drawMainPass(renderPassDescriptor: MTLRenderPassDescriptor, commandBuffer: MTLCommandBuffer) {
        ...
        for (nodeIndex, node) in nodes.enumerated() {
            ...
            for submesh in mesh.submeshes {
                ...
                renderCommandEncoder.setVertexTexture(material?.displacementTexture, index: 0)
                ...
                renderCommandEncoder.drawIndexedPatches(numberOfPatchControlPoints: submesh.patchControlPointCount,
                                                        patchStart: 0,
    ...
    var mostRecentPreferredDevice: MTLDevice?
